...
# 1. Image Analysis
# ------------------------
async def analyze_image(model: str, image_data: str, analysis_type: str, include_conf: bool, lang: str) -> Dict[str, Any]:
    """
    ...
    payload = {"model": model, "prompt": prompt, "images": [image_data], "stream": False}

    result = await _post("generate", payload)
    return {"description": result.get("response", "").strip(), "tags": [], "objects": [], "confidences": []}


# ------------------------
# 2. Document Extraction
# ------------------------
async def extract_document(model: str, document_data: str, extraction_type: str, preserve_layout: bool, extract_tables: bool, extract_images: bool, output_format: str) -> Dict[str, Any]:
    """
    ...
    payload = {"model": model, "prompt": prompt, "images": [document_data], "stream": False}
...
# ------------------------
async def vqa(model: str, image_data: str, question: str, context: Optional[str], answer_type: str, conf: float) -> Dict[str, Any]:
    """
...
# ------------------------
async def ocr(model: str, image_data: str, languages: List[str], preserve_formatting: bool, handwriting: bool, conf_filter: bool, bboxes: bool, regions) -> Dict[str, Any]:
    """
...
# ------------------------
async def charts(
    model: str, image_data: str, chart_type: str = "auto", extract_data: bool = True, extract_labels: bool = True, extract_legend: bool = True, output_format: str = "json"
) -> Dict[str, Any]:
    """
    Analyzes chart or graph images and extracts data series.
    """
    prompt = "Analyze the chart and describe its data trends, labels, and legend. " "If possible, summarize data values in a JSON-like structure."
...
# ------------------------
async def tables(
    model: str, image_data: str, extract_structure: bool = True, extract_content: bool = True, preserve_formatting: bool = True, output_format: str = "csv", header_detection: bool = True
) -> Dict[str, Any]:
    """
    Extracts tables from a document or image using Granite Vision.
    """
    prompt = "Extract any tables visible in the image and output them in CSV format. " "Include headers if detected."
